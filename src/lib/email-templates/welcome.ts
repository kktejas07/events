...
export function renderWelcomeEmail(data: WelcomeData) {
  const body = wrapContent(
    ...
    textBlock(
      `We're thrilled to have you join the Events Platform community. Your account has been created successfully with ${data.email}.`
    ) +
      textBlock(
  ...
  return baseEmailLayout(body, {
    title: "Welcome to Events Platform",
